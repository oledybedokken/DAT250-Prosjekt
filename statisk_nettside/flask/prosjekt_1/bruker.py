...
def slett_konto():
    ...
    if not finnes or email != bruker["email"]:
        return am.jsonify({"msg": "Feil email/passord eller autorisering kode"}), 409

    result = am.bruker.slett_konto({"email": email})
    if result.slettet:
        return am.jsonify({"msg": f"e-mailen <{email}> er slettet!"}), 200
    return am.jsonify({"msg": f"e-mailen <{email}> ikke finnes!"}), 409