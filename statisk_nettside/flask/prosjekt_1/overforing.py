...
def transfer():
    ...
    if not data:
        return am.make_response("OBS! Ingen data", 400)
    try:
    ...
    if not isinstance(sum_mengde, float) and not isinstance(sum_mengde, int):
        return am.jsonify({"msg": "uvanlig input"}), 400
    ...
    bruker_fra = withdraw(nav_bruker, konto_fra, sum_mengde,
                        melding_fra)
    if not bruker_fra:
        return am.jsonify({"msg": "kunne ikke overføre til konto"}), 409
    ...
    return am.jsonify(
        {"fra": konto_fra, "til": konto_til, "sum_mengde": sum_mengde}), 200